...
public:
  ...
  detId_t detID(fedId_t fed, fedCh_t channel) const { return detID_[fed-kFedFirst][channel]; }
  APVPair_t iPair(fedId_t fed, fedCh_t channel) const { return iPair_[fed-kFedFirst][channel]; }

  float noise(fedId_t fed, fedCh_t channel, stripId_t strip) const
    { return noise_[fed-kFedFirst][channel*kStripsPerChannel + (strip % kStripsPerChannel)]; }

  float gain(fedId_t fed, fedCh_t channel, stripId_t strip) const
    { return gain_[fed-kFedFirst][channel*kStripsPerChannel + (strip % kStripsPerChannel)]; }

  bool bad(fedId_t fed, fedCh_t channel, stripId_t strip) const
